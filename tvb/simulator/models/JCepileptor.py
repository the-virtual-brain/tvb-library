...
class JC_Epileptor(ModelNumbaDfun):
    r"""
        ...
        ":math:`I_{rs}`"
        Variables of interest to be used by monitors: :math:`p * (-x_{1} + x_{2}) + (1 - p) * x_{rs}`
            ...
            .. math::
                ...
                \dot{y_{rs}} &=& d_{rs} \dfrac{(b_{rs}  x_{rs} - \beta_{rs} y_{rs} + a_{rs})}{\tau_{rs}}
    ...
    ui_configurable_parameters = ["Iext", "Iext2", "r", "x0", "slope", "tau_rs", "a_rs", "b_rs", "I_rs", "d_rs", "e_rs",
                                  "f_rs", "alpha_rs", "beta_rs", "gamma_rs"]
...
def _numba_dfun(y, c_pop,
    ...
    ydot[6] = d_rs[0] * tau_rs[0] * (
                alpha_rs[0] * y[7] - f_rs[0] * y[6] ** 3 + e_rs[0] * y[6] ** 2 + gamma_rs[0] * I_rs[0] + gamma_rs[0] *
                K_rs[0] * c_pop3 + lc_1[0])
