...
class MetaType(abc.ABCMeta):
    ...
    def __call__(cls, *args, **kwds):
        ...
        for name, attr in inst.trait.items():
            try:
        ...
        for name, attr in kwdtraits.items():
            try:
...
class Type(object, metaclass=MetaType):
    ...
    def __repr__(self):
        ...
        reprstr = ['%s=%s' % (k, v) for k, v in list(reprinfo.items())]
    ...
    def _repr_html_(self):
        ...
        if info is None or len(info) == 0:
            info = {key: getattr(self, key) for key in list(self.trait.keys())}
