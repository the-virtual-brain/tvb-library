...
class MinNormPerturbModel(PerturbationModel):
    def __init__(self, radius=1.5, perturbtype='C'):
        super(MinNormPerturbModel, self).__init__(self,
                                                  radius=1.5,
                                                  perturbtype='C')
    ...
    def dcperturbation(self, A, radius=None):
        ...
        # initialize array to store all the minimum euclidean norms, vecs and
        # freqs
        ...
        for inode in range(0, numchans):
            ...
            # store the minimum norm perturbation achievable, vector and
            # frequency
    ...
    def gridsearchperturbation(self, A, searchnum=51):
        ...
        wspace = np.append(
            top_wspace, op_wspace[1:len(top_wspace) - 1], axis=0)
        ...
        for inode in range(0, numchans):
            ...
            # store the perturbation vector(s)/frequency(s) that produces
            # minimum euclidean norm for this inode (channel)
            if len(min_indices) == 1:
                ...
                # store the position on instability circle that the minimum
                # norm perturbation occurs
                ...
                for idx, min_index in enumerate(min_indices):
                    ...
                    if idx == 0:
                        ...
                        to_insert_vec = np.concatenate(
                            (to_insert_vec, vec), axis=1)
